...
#### 1. 出力ファイルのクリーンアップ戦略を「日数ベース」から「件数ベース」に変更 (v0.6.2)

**Issue**

- 旧実装では `yyyymmdd_HHmmss` プレフィックスが 7 日より古いファイルを削除していたが、実行頻度によっては大量のファイルが残り続ける or 必要なファイルが早期に消える問題があった。
- また、クリーンアップをファイル書き出し**前**に実行していたため、初回実行など出力先ディレクトリが空の状態から始まったときでも問題なかったが、出力後に残件数を確認できなかった。

**Changes**

- `OUTPUT_FULL_FILES_TO_KEEP = 30` 定数を追加し、保持する `*__full.txt` の最大件数を定義。
- `_cleanup_old_output_files()` の保持戦略を「7 日超過で削除（時刻ベース）」から「最新 30 件の `__full.txt` に対応する全出力ファイルを残す（件数ベース）」に変更。戻り値に削除件数（`int`）を追加。
- `_output_file_sort_key()` ヘルパー: `yyyymmdd_HHmmss` プレフィックスで datetime をパースし、パース失敗時は mtime にフォールバック。
- `_generated_output_prefix()` ヘルパー: `*__full.txt` および `*__NNofMM.txt` のファイル名からプレフィックスを抽出し、セットで保持判定に使用。
- クリーンアップ実行タイミングをファイル書き出し**前**から**後**に移動。verbose モード時に削除件数を stderr に出力。
- 不要になった `timedelta` インポートを削除。
